...
## night 7
## night 7 (s0 = preseason friendlies, played)
...
## night 10
## night 8 (round 4 prep, buzzer rule landed)
CHANGES (v5): (1) KICKOFF — only Hare charges the centre ball; Tortoise drops to backstop at (ogx+3.2 toward centre, 0). Motive: unforced falls at t~4.5 AND ~8.7 in ALL THREE s3 matches (m4/m10/m15 digests) — our two players sprint-kicking the same ball collide. (2) BUZZER override — new rule: at each half's end power dies but ball rolls 5-10s and goals count. In last 2.5s of a half, if within 1.6m of ball: their half -> shoot at goal (unblockable); our half -> blast to midfield wing (loose ball at our net is a grenade). Uses t0_rem/2 as half length.
VERIFY: lint CLEAR; 90s practice mirror ran clean, score 0-2 (away won). NB: old mirror was 1-0 home — home edge flipped in mirror. Deterministic, so real signal, but symmetric code; likely single-Hare wedge loses kickoff duel to the away Hare. WATCH round 4 (m19 away at Singularity United): restart won/lost line, and whether kickoff falls at 4.5/8.7 are GONE. If we start losing kickoff possession badly, consider Tortoise at a nearer support spot (not full backstop) instead of reverting to the colliding wedge.
QUEUE: (1) m19 autopsy — falls, restarts, buzzer moments (match.json 'buzzers', goals 'after_buzzer'). (2) honest_latency pressing preset vs slow clubs (DeepSeek 2.3s, GLM 2.4s) still unbuilt. (3) m10 6-4 vs Sol and m15 loss never fully autopsied.
